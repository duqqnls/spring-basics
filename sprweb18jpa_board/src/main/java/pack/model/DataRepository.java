...
public interface DataRepository extends JpaRepository<Board, Integer>{
	...
	@Query("select b from Board b where b.title like %:searchValue%") // as 생략 + 위치기반 (이름 매핑)
	List<Board> searchLike2(@Param("searchValue") String searchValue);

	// 추가할 때 가장 큰 번호 얻기
	@Query("select max (b.num) from Board b") // Board 엔터티를 통해 얻기
	int maxNum();
	
	// 상세보기 할 때 조회수 증가
	// 내부적으로 JPA는 벌크 연산(update, delete, insert)을 한다.
