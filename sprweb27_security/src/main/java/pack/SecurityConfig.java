...
public class SecurityConfig { // 기본적인 웹 보안 구성을 설정
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
		...
		http
			...
			.logout(logout ->							   // logout은 인증없이 누구든 접근 허용
				logout
					.logoutUrl("/logout")
	...
	@Bean
	public UserDetailsService userDetailsService() { 	   // method
		UserDetails user = User.builder()
								.username("duqqnls")
								.password(passwordEncoder().encode("123"))   // 비밀번호 암호화
								.roles("USER") 			   // default user 역할
								.build();				   // 사용자명과 비밀번호를 역할 설정
