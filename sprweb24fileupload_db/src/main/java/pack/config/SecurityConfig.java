...
// 보안설정 자동화, 인증 및 권한 부여, 비밀번호 암호화, CSRF 공격 보호, 기본 로그인 폼 제공, user라는 기본 사용자 계정 생성...

public class SecurityConfig { 
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{ // 보안과 관련해서 chain
		
		http
		.authorizeHttpRequests((requests) -> 
			requests.requestMatchers("/").permitAll().
			anyRequest().authenticated()) 	// 루드 경로는 모두 허용. 그 외는 인증 필요
		.formLogin((form) -> form.loginPage("/login").permitAll()) // 커스텀 로그인 페이지 설정
		.logout((logout) -> 				// 로그아웃 설정을 정의	
			logout.logoutUrl("/logout")	
			.logoutSuccessUrl("/") 			// 로그아웃 성공하면 경로
